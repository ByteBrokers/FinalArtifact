...
const Game3D = ({ characterData, initialGameState, userId, onLogout, onGoHome }: Game3DProps) => {
  ...
  const createPlayer = (charData: CharacterCustomizationData = currentCharacterData) => {
    ...
    const eyeMaterial = new THREE.MeshLambertMaterial({ color: 0x000000 });
    const leftEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    leftEye.position.set(-0.2 * charData.width, 3.15 * charData.height, 0.801 * charData.height);
    player.add(leftEye);

    const rightEye = new THREE.Mesh(eyeGeometry, eyeMaterial);
    if (facialExpression === "wink") {
      rightEye.scale.y = 3;
      rightEye.position.set(0.2 * charData.width, 3.15 * charData.height, 0.801 * charData.height);
    } else {
      rightEye.position.set(0.2 * charData.width, 3.15 * charData.height, 0.801 * charData.height);
    }
    player.add(rightEye);
    ...
    if (facialExpression === "sad") {
      mouth.position.set(0, 2.8 * charData.height, 0.801 * charData.height);
      mouth.rotation.z = Math.PI;
    } else if (facialExpression === "angry") {
      mouth.position.set(0, 2.82 * charData.height, 0.801 * charData.height);
      mouth.rotation.z = Math.PI * 0.05;
    } else if (facialExpression === "surprised") {
      mouth.position.set(0, 2.82 * charData.height, 0.801 * charData.height);
    } else {
      mouth.position.set(0, 2.85 * charData.height, 0.801 * charData.height);
    }
    player.add(mouth);
